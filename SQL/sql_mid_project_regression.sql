...
-- We have skipped the 3rd step as we have imported the data in a different way. With table data import wizard
...
ALTER TABLE regression_data RENAME house_price_data; -- Rename table 
SELECT* FROM house_price_data;
-- Data was imported properly
...
SELECT DISTINCT house_price_data.condition from house_price_data order by house_price_data.condition; 
...
ORDER BY bedrooms;
    ...
    # Use an alias to change the name of the second column.
SELECT bedrooms , 
	AVG(sqft_living) as Average_Living_Space_SQF 
FROM house_price_data 
GROUP BY bedrooms
ORDER BY bedrooms;
...
-- We can see that there is a non-linear correlation between 'grade' and 'condition'. For Condition 1,2 and 3 Avg. grade is increasing and for grade 4 and 5 slightly decreases.
...
SELECT* 
FROM house_price_data 
WHERE price >= (SELECT 2*ROUND(AVG(price),2) 
				FROM house_price_data) -- this table gets the twice average price of the data in the table
ORDER BY price DESC;
...
-- Query to get average price for both categories. 3 and 4 bedrooms
...
SELECT (SELECT ROUND(AVG(price),2) AS average FROM house_price_data WHERE bedrooms=4) as four_bedroom_average,
	...
	((SELECT ROUND(AVG(price),2) AS average FROM house_price_data WHERE bedrooms=4) - (SELECT ROUND(AVG(price),2) AS average FROM house_price_data WHERE bedrooms=3)) as difference -- difference calculation
FROM house_price_data 
GROUP BY four_bedroom_average,three_bedroom_average,difference;
...
-- if yr_renovated is different than 0 then the property was renovated
